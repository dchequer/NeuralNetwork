...
class Layer:
    def __init__(self, activation:Activation, inNodes:int, previousLayerSize:int, **kwargs):
        '''
        ...
        self.bias: float = np.random.rand(inNodes, 1) #List[m] of biases | one bias for each neuron in current layer
        self.weights: np.ndarray = np.random.rand(inNodes, previousLayerSize) #m Lists of List[n] of weights | one weight for each neuron in previous layer, for each node in current layer
    ...
    def backward(self, learnRate: float, regularization: float, momentum: float) -> None:
        '''
    ...
    def updateGradients(self, inputs: np.ndarray, nodeValues: np.ndarray) -> None:
        ...
        for nodeToIdx, nodeValue in enumerate(nodeValues):
            for nodeFromIdx, nodeInput in enumerate(inputs):
                ...
                self.costGradientWeights[nodeToIdx][nodeFromIdx] += derivativeCostWRTWright
    ...
    def hiddenNodeValues(self, layerData: List(np.ndarray), rightLayer: Layer, rightNodeValues: np.ndarray) -> np.ndarray:
        '''
        ...
        for leftIdx, leftNodeValue in enumerate(leftNodeValues):
            ...
            for rightIdx, rightNodeValue in enumerate(rightNodeValues):
                weightedInputDerivative = rightLayer.weights[leftIdx][rightIdx] # dz/da = w[from][to]
